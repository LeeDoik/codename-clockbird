...
## 반입된 것 — 22인 (본부 4 · 저택 10 · 거리 8)

전부 남향 정지 그림 한 장씩이다. 예외 둘: 사무엘(북동) · 아이리스(남동) — 기획
배치도에서 그 둘만 비스듬히 서 있다.

| 스테이지 | 인물 → 게임 id | 내보내기 원본 |
| --- | --- | --- |
| 본부 | 01 브란트 → `officer` · 02 에이다 → `t1`(색상) · 03 토마스 → `t2`(분류) · 04 리아 → `t3`(특징) | `design/NPC/pixellab/` |
| 저택 | 14 세드릭 `butler` · 15 클라라 `cook` · 16 오스카 `gardener` · 17 에밀리 `washer` · 23 사무엘 `shelver` · 19 피터 `diner` · 20 조셉 `cleaner` · 22 올리버 `clerk` · 18 빅터 `guard` · 21 헤나 `coach` | `design/NPC/pixellab2/` |
| 거리 | 06 엘라 `maid` · 07 가레스 `engineer` · 08 실비아 `smuggler` · 09 루카스 `musician` · 13 아이리스 `florist` · 12 벤 `porter` · 11 미사 `baker` · 10 노아 `newsboy` | `design/NPC/pixellab3/` |

⚠ 본부 id 는 **담당 축의 키**이지 인물이 아니다 (에이다=색상=`t1`). 거리 id 는 초상
파일명과 짝이라 직업과 어긋난 자리가 있다 (`maid` 는 여급 엘라, `musician` 은 의사 루카스).
`design/characters/portrait-map.md` 가 대응표의 단일 출처다.

**05 에이던(`watchmaker`)만 PixelLab 판이 없다.** 예전 256px 12프레임 아이들 시트를
그대로 쓴다 — 기획 배치도에 그려진 것도 그 그림이라 갈아끼울 이유가 없었다. 거리에서
혼자만 숨을 쉰다.

굽는 것은 `node scripts/import-npc-sprites.js` 한 줄이고, 실측값은
`src/client/entities/npcSprite.js` 가 단일 출처다.

**PixelLab 은 인물마다 프레임 크기를 다르게 준다** (188~228). 그대로 쓰면 원점을 하나로
잡을 수 없어 인물마다 발 높이가 어긋나므로, 반입 스크립트가 **공통 228칸에 공통
땅선(y=170)으로 다시 앉힌다**. 그래서 씬이 드는 값은 원점 하나(전원 공유) + 인물
키(사람마다 다름)뿐이고, 화면에서는 전원이 그 맵의 `charHeight` 로 선다.

⚠ 공통 프레임은 **표에 있는 전원 중 가장 큰 원본**에 맞춘다. 인물을 더하면 프레임이
커질 수 있고 그러면 세 스테이지의 그림이 전부 다시 구워진다 — 거리 여덟이 들어오면서
200→228 로 늘었다(가레스가 228). 스크립트가 찍어 주는 값을 `npcSprite.js` 로 옮겨
적기만 하면 되지만, 그 커밋에 `-south.png` 22장이 통째로 들어온다.
...
→ `scripts/import-actor-sprites.js` 가 그 처리를 한다. **걸어다니는 것**(주인공·순찰 로봇)은
전부 이 스크립트를 거친다. 제자리에 서 있기만 하는 NPC 는 `import-npc-sprites.js` 쪽이다 —
방향이 하나뿐이라 묶음 정렬 대신 **여럿을 공통 프레임에 함께** 앉힌다.
