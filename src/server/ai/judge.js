...
export async function judgeGuess({ codeWord, guess }) {
  ...
  const message = await anthropic.beta.messages.parse({
    model: MODEL_JUDGE,
    max_tokens: 500,
    // 난이도 하향: 예전엔 "애매하면 오답"이라 아깝게 맞힌 판이 자주 무산됐다.
    // 같은 사물을 가리키는 게 분명하면 표현 차이는 눈감아 주되, 범주 경계는 지킨다
    // (여기서 "관련 개념"까지 정답 처리하면 추리가 사라져 게임이 죽는다 — 그 선은 유지).
    system: `너는 단어 맞히기 게임의 심판이다. 플레이어의 답이 정답 단어와 "사실상 같은 것"을 가리키는지 판정하라.

정답으로 인정:
- 동의어, 표기 변형, 띄어쓰기 차이, 어미 차이
- 사실상 같은 사물을 가리키는 다른 이름 (플레이어가 그것을 알아본 게 분명한 경우)
