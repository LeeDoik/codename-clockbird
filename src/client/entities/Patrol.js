...
/**
 * 순찰 경로 — 거리 맵(70×35, 팀이 준 새 광장 도면)의 큰 축을 따라 왕복한다.
 *
 * 이전 60×48 도면(docs/archive/스테이지1 거리 맵.dc.html 의 baseGuards)은 이번에
 * 새 도면으로 완전히 갈아엎으며 좌표를 다시 잡았다 — 직선 보간만 하고 경로탐색은
 * 안 하므로, 두 점을 잇는 직선이 건물에 안 걸리는 통로를 새 그림에서 다시 골랐다.
 *
...
export const PATROL_ROUTES = {
  avenue: [at(34, 3), at(34, 31)], // 세로 축 — 시계탑 광장을 관통하는 중앙 대로
  crossing: [at(4, 17), at(30, 17)], // 가로 축 — 꽃가게·화로 앞 서쪽 큰 길(동쪽은 건물이 막아 짧게 잡았다)
  wharf: [at(10, 25), at(58, 25)], // 아래쪽 가로 축 — 화로·대장간 구역 앞
  reinforce: [at(16, 9), at(16, 17)], // 증원 — 경계 2 이상에서만, 꽃가게·화로 사이 골목
};
