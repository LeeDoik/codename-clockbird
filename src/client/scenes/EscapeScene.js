...
/**
 * 심문을 통과한 순간 에바가 보내 주며 하는 말 — **이 판의 결말 대사**다.
 *
 * 이 한마디가 없으면 에바는 판정이 뜨자마자 말없이 돌아서서 사라진다. 놓아 준
 * 것인지 흥미를 잃은 것인지 알 수 없고, 뒤따르는 독백 "(진짜로 로봇이 나를 놓아
 * 준다고?!)" 도 받을 말이 없어 허공에 뜬다 (2026-08-08 피드백).
 *
 * 원래 이 말은 심문 패널 안에 있었다(robotInterrogation 의 옛 WIN_LINE). 패널이
 * 닫힌 직후 필드에서 또 보내 주는 말을 하게 되면서 같은 말을 두 번 듣게 돼,
 * **패널은 판정 도장만 찍고 말은 여기로 모았다** — 마주 선 얼굴을 보고 듣는 편이
 * 낫다. 그래서 옛 대사의 알맹이("제법인걸"·"약속은 약속이니까")를 여기가 물려받았다.
 *
 * ⚠ "약속은 약속이니까" 를 빼지 말 것. 에바는 지하 수로에 숨은 사람이 누구인지
 *   처음부터 알고 있고(escape.json 의 child.backstory — "이런 곳에서 발견된 걸 보면
 *   뻔하지만, 나의 심문에서 빠져나간다면 모르는 척 넘어가 주려고 한다"), 이 판은
 *   그녀가 혼자 정해 놓고 시작한 놀이다. 저 한 마디가 "왜 살려 보내는가"에 대한
 *   유일한 답이라, 없으면 연기를 잘했다고 로봇이 봐주는 이상한 장면이 된다.
 */
const EVA_FAREWELL = '"오… 제법인걸. 흥미롭군.\n약속은 약속이니까 — 그냥 가봐도 좋아."';
