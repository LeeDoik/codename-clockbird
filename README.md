...
> 기획 문서: [`docs/NAN2026_계획서.md`](./docs/NAN2026_계획서.md) · 문서 전체 색인: [`docs/README.md`](./docs/README.md)
...
Windows 사용자는 **`studio.bat` 더블클릭** 한 번이면 됩니다 (서버 기동 + 브라우저 자동 열기).
...
12개(`src/data/prompts/*.txt`) 전부입니다. **저장하기 전에** 초안 그대로 미리 돌려볼 수
...
스테이지 3 의 양쪽 판정이 나란히 나오는 이유는 이 심문의 설계가 "시스템은 신분 단어를
...
2. ~~튜토리얼 맵을 붙일 때 미리 정리할 것~~ → **완료 (W2)**:
   - 캔버스는 현재 **1920×1080 (16:9)**. 월드는 메인 카메라가 2배 줌으로 그리고
