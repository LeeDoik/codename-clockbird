...
2. 튜토리얼 맵을 붙일 때 미리 정리할 것:
   - **캔버스 920x600 은 32의 배수가 아니다** (28.75 x 18.75 타일). 타일맵을 깔면
     가장자리가 잘린다. 896x576(28x18) 로 바꾸거나 카메라를 도입해야 한다.
   - 맵 데이터 없음. 튜토리얼 1개면 Tiled 없이 손으로 짠 2D 배열 JSON 으로 충분하다.
   - 필요한 최소 아트 9장: 바닥1 + 벽1 + 플레이어1 + 동료5 + 시민1 (전부 32x32).
     현재 벽 타일과 캐릭터 스프라이트가 전혀 없다. 바닥은 `check-tile.js` 참고
     (기존 시안 2종 모두 아직 FAIL 항목이 있다).
